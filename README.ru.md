...
> In-Depth Protection (IDP) - собственная схема полнодискового шифрования, устойчивая к атакам на TPM. Добавляет дополнительный слой криптографической защиты поверх Storage Root Key (SRK). Использует KDF argon2id и AES GCM (256 бит). Реализует поддержку ложного PIN-кода с привязкой к TPM. 
